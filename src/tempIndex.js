    ...
    if (!isNil(customPageNumber)) {
      ...
      if (!isEqual(componentIndex, customPageNumber)) {
        if (!isNil(containers[customPageNumber]) && !isScrolling) {
          ...
          if (pageOnChange) {
            pageOnChange(customPageNumber + 1);
          }

          if (
            isNil(containers[customPageNumber + 1]) &&
            !isNil(children[customPageNumber + 1])
          ) {
          ...
          if (!isNil(children[customPageNumber + 1])) {
            newComponentsToRenderLength++;
          }
